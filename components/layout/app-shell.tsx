    ...
    <div className="flex min-h-full flex-1 bg-background">
      <aside className="fixed inset-y-0 left-0 hidden w-64 flex-col bg-sidebar py-5 md:flex">
        <div className="px-5 pb-6">
          <Marca
            invertido
            bajada={usuario.role === "ADMIN" ? "Administración" : "Vendedor"}
          />
        </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col md:pl-64">
        <header className="sticky top-0 z-10 flex h-16 items-center justify-between gap-4 border-b border-border/70 bg-background/85 px-4 backdrop-blur-sm md:px-8">
          {zona ? (
